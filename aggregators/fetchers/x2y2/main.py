...
class X2Y2Fetcher(BaseFetcher):
	...
	def get_loans(self, counter):
		...
		return [X2Y2Loan(block_time=datetime.utcfromtimestamp(float(x['blockTimestamp'])),
		             ...
		             loan_end=datetime.utcfromtimestamp((int(x['loanDetail_loanStart']) + int(x['loanDetail_loanDuration']))),
	...
	def get_liquidations(self, counter):
		...
		return [X2Y2Liquidation(
			...
			nft_asset=x['nftAsset'],
			nft_token_id=x['nftTokenId'],
			loan_liquidation_time=datetime.utcfromtimestamp(float(x['loanLiquidationDate'])),
			loan_maturity_time=datetime.utcfromtimestamp(float(x['loanMaturityDate'])),
	...
	def process_loans(self):
		...
		X2Y2Loan.objects.bulk_create(raw_loans, batch_size=self.BATCH_SIZE, ignore_conflicts=self.IGNORE_CONFLICTS)

	def process_repaids(self):
		raw_repaids = self.get_all(self.get_repaids)
		X2Y2Repaid.objects.bulk_create(raw_repaids, batch_size=self.BATCH_SIZE, ignore_conflicts=self.IGNORE_CONFLICTS)

	def process_liquidations(self):
		raw_liquidations = self.get_all(self.get_liquidations)
		X2Y2Liquidation.objects.bulk_create(raw_liquidations, batch_size=self.BATCH_SIZE, ignore_conflicts=self.IGNORE_CONFLICTS)

	def process_cancels(self):
		raw_cancels = self.get_all(self.get_nonce_cancelleds)
		X2Y2NonceCancelled.objects.bulk_create(raw_cancels, batch_size=self.BATCH_SIZE, ignore_conflicts=self.IGNORE_CONFLICTS)
