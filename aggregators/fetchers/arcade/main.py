...
from aggregators.models import X2Y2Loan, X2Y2NonceCancelled, X2Y2Repaid, X2Y2Liquidation, ArcadeLoan
...
class ArcadeFetcher(BaseFetcher):
	...
	def get_loans(self, counter):
		...
		return [
			ArcadeLoan(
				...
				loan_repay_amount=self.get_loan_repay_amount(x['interestRate'], x['principal']),
	...
	def process_loans(self):
		...
		ArcadeLoan.objects.bulk_create(raw_loans, batch_size=self.BATCH_SIZE, ignore_conflicts=self.IGNORE_CONFLICTS)
